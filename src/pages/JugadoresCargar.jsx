...
export default function JugadoresCargar() {
  ...
  useEffect(() => {
    if (torneoId) {
      fetch(`http://localhost:3001/api/torneos/${torneoId}/jugadores`)
        .then(res => res.json())
  ...
  const handleAgregarJugador = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/torneos/${torneoId}/jugadores`,
        {
          method: 'POST',
  ...
  const handleEliminarJugador = async (jugadorId) => {
    if (window.confirm('¿Eliminar este jugador?')) {
      try {
        await fetch(`http://localhost:3001/api/jugadores_torneo/${jugadorId}`, {
          method: 'DELETE',
        });
